...
class GroqKeyPool:
    ...
    Default model: "openai/gpt-oss-20b". Groq deprecated llama-3.1-8b-instant
    and llama-3.3-70b-versatile (announced June 17, 2026), recommending
    openai/gpt-oss-20b and openai/gpt-oss-120b as replacements — this also
    fixed a real reliability problem observed during testing (inconsistent
    field extraction between identical runs), consistent with hitting a
    model already being phased out. Pass model="openai/gpt-oss-120b" for
    calls where completeness matters more than speed (e.g. multi-field/
    table extraction) — see Extractor, which does this by default.
    """

    def __init__(self, api_keys: Optional[list[str]] = None, model: str = "openai/gpt-oss-20b"):
    ...
    def complete_json(
        ...
        `model` overrides this pool's default for just this call — e.g. the
        Extractor requests the larger "openai/gpt-oss-120b" for better
        multi-field/table completeness, while the Classifier uses the pool's
        faster default since classification is a simpler task.
        """
        last_error = None
