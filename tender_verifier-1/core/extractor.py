...
RULES_PATH = CONFIG_DIR / "document_rules.json"
...
class Extractor:
    ...
    def _extract_llm_fields(self, text: str, doc_type: str, llm_field_names: list[str], rule: dict) -> dict:
        ...
        system_prompt = (
            ...
            "Respond as a JSON object with EXACTLY these keys — every key listed below MUST appear "
            "in your response, even if the value is null:\n" + "\n".join(field_descriptions) + "\n\n"
            "Rules:\n"
            "- If a field is not present on this page, use null for it — but the KEY must still be present.\n"
            "- For any field described as a list/table (e.g. multiple financial years), extract EVERY "
            "row visible on the page, not just one. Do not stop after the first match.\n"
            "- Do not skip a field just because a similar-looking field was already extracted — "
            "e.g. an 'average' or 'total' figure is a separate field from the per-year breakdown, "
            "extract both if both appear.\n"
            "- Double-check before responding that your JSON object contains all the requested keys."
        )
        user_prompt = text[:3000]  # hard cap — this must be page-level, not document-level

        result = self.groq_pool.complete_json(
            system_prompt, user_prompt, max_tokens=1000, model="openai/gpt-oss-120b"
        )
        return result or {}
